...
function StatBox({ label, value }: { label: string; value: any }) {
  return (
    <div className="text-center p-2 rounded-md bg-card/80 border border-amber-500/10">
...
function FieldDisplay({ label, value }: { label: string; value: any }) {
  ...
  return (
    <div className="space-y-1">
      <Label className="text-xs text-amber-400/70 capitalize">{label.replace(/_/g, ' ')}</Label>
    ...
    <div className="space-y-4">
      ...
      {(data.size || data.type || data.challenge_rating) && (
        <div className="flex gap-2 flex-wrap">
          {data.size && <Badge variant="outline" className="border-amber-500/30 text-amber-300">{data.size}</Badge>}
          {data.type && <Badge variant="outline" className="border-amber-500/30 text-amber-300">{data.type}</Badge>}
          {data.challenge_rating && <Badge variant="outline" className="border-red-500/30 text-red-300">CR {data.challenge_rating}</Badge>}
        </div>
      )}
      {/* Ability scores */}
      {hasStats && (
    ...
    elements.push(
      <div key={group.label} className="space-y-2">
        <div className="flex items-center gap-1.5">
          {group.icon && <span className="text-amber-400">{group.icon}</span>}
          <span className="text-xs font-semibold uppercase tracking-wider text-amber-300/80">{group.label}</span>
        </div>
  ...
  if (remaining.length > 0) {
    elements.push(
      <div key="__remaining" className="space-y-2">
        <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Other Details</span>
        <Separator className="bg-amber-500/10" />
        <div className="grid gap-2.5">
