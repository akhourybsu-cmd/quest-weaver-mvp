...
import { ManageEffectSchema, validateRequest } from '../_shared/validation.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    const validation = validateRequest(ManageEffectSchema, body, corsHeaders);
    if (!validation.success) {
      return validation.response;
    }

    const { action, encounterId, effectData, effectId } = validation.data;
